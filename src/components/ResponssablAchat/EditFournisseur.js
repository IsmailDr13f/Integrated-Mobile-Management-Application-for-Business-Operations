...
const EditFournisseur = ({ fid, CloseEvent }) => {

    const [Réference, setRef] = useState();
    const [Abrégé, setAbr] = useState();
    const [Qualité, setQual] = useState();
    ...
    useEffect(() => {
        ...
        console.log(fid.Abrégé);
        setAbr(fid.Abrégé);
        setQual(fid.Qualité);
        setComCol(fid.Compte_collectif);
        setAdr(fid.Adresse);
        setComp(fid.Complément);
        setCoPo(fid.Code_postale);
        setVille(fid.Ville);
        setPays(fid.Pays);
        setTele(fid.Telephone);
        setEmail(fid.Email);
    }, []);
