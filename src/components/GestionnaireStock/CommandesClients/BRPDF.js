...
const BRPDF = ({ bon }) => {
    const qrCodeValue = `fac: ${bon.NFBR}\nClient: ${bon.Client}\nDate: ${bon.DateR}`;
  ...
  return (
    <Document>
      <Page>
          ...
          <Text style={styles.title}>Bon de retoure Num : {bon.NBR}</Text>
          ...
          <View style={styles.productTable}>
            ...
            {bon && bon.productR && bon.productR.map((item, index) => (
          ...
          <Text style={{fontSize:12,fontWeight:'bold',marginLeft:20,marginBottom:10,marginTop:0}}>Adresse de livraison: {bon.adrees}</Text>
          <Text style={{fontSize:12,fontWeight:'bold',marginLeft:20}}>Date de Retoure: {bon.DateR}</Text>
            <Text style={{ fontSize: 10, fontWeight: 'bold',marginTop:30, marginRight:30,marginBottom:20,marginLeft:20, }}>Cachet et signature </Text>
